...
{
	int length = CEIL(RELIC_BN_BITS, 8) * 4;
	char data[length + 1];
...
relicxx::G1 G1::setBytes(uint8_t bin[], bool compress) const
{
	relicxx::G1 g;
	PairingGroup gr_group;
	relicxx::G1 g2 = gr_group.randomG1();
	unsigned int l = g1_size_bin(g2.g, compress);
...
GT power(const GT &g, const ZR &zr)
{
	...
	RELICXX_ZRunconst(zr, zr1);
	if (zr == ZR(-1))
	{ // find efficient way for comparing bn_t to ints
		// compute inverse
		return -g;
	}
	else
